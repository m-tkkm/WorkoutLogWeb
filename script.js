...
function switchSection(sectionId) {
    ...
    const sectionEvents = {
        ...
        "exercise-section":
            "view_exercise_management"
    };

    const eventName =
        sectionEvents[sectionId];
...
function renderPastExerciseSelect(
    preferred = null
) {
    populateExerciseSelect(
        pastExerciseInput,
...
// 種目管理
function createExerciseListItem(
    exercise,
    isStandard
) {
    const item =
...
function updatePreviousRecord() {
    ...
    previousRecordOutput.textContent =
        record
            ? `${formatWeight(
                record.weight
            )}kg × ${
...
function resetPastRecordForm() {
    ...
    addDayRecordForm.classList.add(
        "hidden"
    );
...
function showSelectedDate(
    ...
    selectedDaySection.classList.remove(
        "hidden"
    );
    ...
    if (!records.length) {
        ...
        selectedDayRecords.appendChild(
            empty
        );
    } else {
        records.forEach(
